...
export function Root({ className, ...props }: NumberField.Root.Props) {
	return (
		<NumberField.Root
			className={clsx("flex flex-col items-start gap-1 w-full", className)}
			{...props}
		/>
	);
}
...
export function Group({ className, ...props }: NumberField.Group.Props) {
	return (
		<NumberField.Group className={clsx("flex w-full", className)} {...props} />
	);
}
...
>(function Input(
	...
	return (
		<NumberField.Input
			ref={forwardedRef}
			className={clsx(
				"h-12 w-24 border-t border-b border-gray-200 text-center text-base text-gray-900 tabular-nums focus:z-1 focus:outline focus:outline-2 focus:-outline-offset-1 focus:outline-blue-800 flex-1",
				className,
			)}
			{...props}
		/>
